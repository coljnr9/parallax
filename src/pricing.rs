...
pub async fn fetch_pricing(client: &reqwest::Client) -> std::collections::HashMap<String, CostModel> {
    let mut attempts = 0;
    let max_attempts = 3;
            ...
            .await
        {
            ...
            Err(e) => {
                if attempts >= max_attempts {
                    tracing::error!("Failed to fetch pricing after {} attempts: {}", max_attempts, e);
                    return pricing;
                }
                tracing::warn!("Failed to fetch pricing (attempt {}/{}): {}. Retrying in 2s...", attempts, max_attempts, e);
