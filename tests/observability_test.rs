...
mod tests {
    ...
    async fn setup_test_app() -> axum::Router {
        ...
        sqlx::query("CREATE TABLE signatures (id TEXT PRIMARY KEY, conversation_id TEXT, metadata TEXT)").execute(&db).await.unwrap();
        ...
        let state = Arc::new(parallax::AppState {
            ...
            circuit_breaker: Arc::new(parallax::hardening::CircuitBreaker::new(5, std::time::Duration::from_secs(30))),
        ...
        axum::Router::new()
            ...
            .route("/admin/conversation/:cid", axum::routing::get(parallax::health::admin_conversation))
            .with_state(state)
    }
    ...
    async fn test_admin_unauthorized() {
        ...
        let response = app
            .oneshot(Request::builder().uri("/admin/conversation/test-cid").body(Body::empty()).unwrap())
            .await
            .unwrap();
