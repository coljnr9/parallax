use parallax::db::{cleanup_old_data, init_db};
use tempfile::tempdir;

#[tokio::test]
async fn test_migrations_and_schema() {
    let dir: tempfile::TempDir = match tempdir() {
        Ok(d) => d,
    ...
    assert!(
        !table_names.contains(&"usage_stats".to_string()),
        "usage_stats should have been dropped"
    );
    ...
    let columns: Vec<(i64, String, String, i64, Option<String>, i64)> =
        match sqlx::query_as("PRAGMA table_info(tool_signatures)")
            .fetch_all(&pool)
            .await
        {
            Ok(c) => c,
